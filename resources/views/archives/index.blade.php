        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="mb-4 d-flex align-items-center gap-3 p-3">
                    ...
                    {{-- Year Dropdown --}}
                    <div class="dropdown">
                        <button class="btn btn-sm btn-secondary dropdown-toggle" type="button" id="yearDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                            {{ $year }}
                        </button>
                        <ul class="dropdown-menu" aria-labelledby="yearDropdown">
                            @foreach($years as $y)
                                <li>
                                    <a class="dropdown-item {{ $year == $y ? 'active fw-bold' : '' }}"
                                    href="{{ route('archives', array_merge(request()->query(), ['year' => $y])) }}">
                                        {{ $y }}
                                    </a>
                                </li>
                            @endforeach
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="courseDropdown">
                            <li><a class="dropdown-item" href="{{ route('archives', array_merge(request()->query(), ['course' => null])) }}">All Courses</a></li>
                            @foreach(['BSIT', 'BSCS', 'BLIS'] as $c)
                                <li>
                                    <a class="dropdown-item" href="{{ route('archives', array_merge(request()->query(), ['course' => $c])) }}">
                                        {{ $c }}
                                    </a>
                                </li>
                            @endforeach
                    ...
                    <div class="form-check ms-3">
                        <input class="form-check-input" type="checkbox" value="1" id="rankingCheck"
                               onchange="location = this.checked ? '{{ route('archives', array_merge(request()->query(), ['rank' => 1])) }}' : '{{ route('archives', array_merge(request()->query(), ['rank' => null])) }}';"
                    ...
                    @if(request('course') && request('status') === 'passed')
                        <button class="btn btn-sm btn-primary ms-auto"
                                onclick="window.open('{{ route('archives', array_merge(request()->query(), ['print' => 1])) }}', '_blank')">
