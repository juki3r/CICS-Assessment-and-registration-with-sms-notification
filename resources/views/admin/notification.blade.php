            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                        <div class="responsive-table">
                            <h3 class="fs-3 mb-3">Unread notifications</h3>
                            <span class="text-success" style="font-style: ">Click student to view more details</span>

                            <form method="GET" action="{{ route('admin.notification') }}" class="mb-3">
                                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search..." class="form-control">
                            </form>
                            
                            <div id="notification-results">
                                @include('admin.notification_table')  <!-- Include table view -->
                            </div>
                            
                            <script>
                                document.querySelector('input[name="search"]').addEventListener('input', function () {
                                    // Send AJAX request when search input is changed
                                    fetch(`?search=${this.value}`, {
                                        headers: { 'X-Requested-With': 'XMLHttpRequest' }
                                    })
                                    .then(response => response.text())
                                    .then(data => {
                                        document.getElementById('notification-results').innerHTML = data;
                                    });
                                });
                            </script>
